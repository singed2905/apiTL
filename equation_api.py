...
def validate_excel():
    ...
    try:
        ...
        if is_large:
            # For large files, do minimal validation
            try:
                import openpyxl
                ...
                # Create dummy DataFrame for structure validation
                import pandas as pd
                dummy_df = pd.DataFrame(columns=columns)
                is_valid, missing_cols = excel_processor.validate_excel_structure(dummy_df, operation)
                
                return jsonify({
                    'status': 'success',
                    'message': 'Large file validation completed (structure only)',
                    'data': {
                        'is_valid': is_valid,
                        'is_large_file': True,
                        ...
                        'warning': 'File lớn - chỉ kiểm tra cấu trúc cơ bản'
                    }
                })
                
            except Exception as e:
                return jsonify({'status': 'error', 'message': f'Error validating large file: {str(e)}'}), 500
