...
Business logic for geometry calculations and encoding
Extracted from geometry_api.py for better separation of concerns
"""
...
class GeometryService:
    """Service layer for geometry functionality (config-driven)"""
    ...
    def get_available_shapes(self) -> List[str]:
        """Get list of all available geometric shapes"""
    ...
    def encode_string(self, input_string: str) -> str:
        ...
        Args:
            input_string: Raw input string (may contain LaTeX, numbers, etc.)
            
        Returns:
            Encoded string compatible with Casio calculator keylog format
        """
        if not input_string:
            return ""
        ...
        for idx, rule in enumerate(self.geometry_mappings):
            ...
            try:
                ...
                if text != original and False:  # Set True to enable debug
    ...
    def _validate_shape_data(self, shape: str, data: Dict[str, Any], group: str) -> List[str]:
        """Validate shape-specific data fields"""
        errors: List[str] = []
        ...
        if shape == 'Điểm':
            ...
            req = ['line_A1' if group == 'A' else 'line_A2', 
                   'line_X1' if group == 'A' else 'line_X2']
            for k in req:
                if k not in data:
    ...
    def process_geometry(self, operation: str, shape_A: str, data_A: Dict[str, str],
        ...
        """
        Process geometry calculation and generate keylog
        
        Returns:
            Dict with operation details, encoded values, and keylog
        """
    ...
    def _process_shape_data(self, shape: str, data: Dict[str, str], group: str) -> List[str]:
        """Process and encode shape-specific data"""
        if shape == 'Điểm':
    ...
    def _process_line_data(self, data: Dict[str, str], group: str) -> List[str]:
        """Process line point and direction vector"""
    ...
    def _process_plane_data(self, data: Dict[str, str]) -> List[str]:
        """Process plane equation coefficients"""
        coeffs = [
            data.get('plane_a', '0'),
            data.get('plane_b', '0'),
            data.get('plane_c', '0'),
    ...
    def _process_circle_data(self, data: Dict[str, str]) -> List[str]:
        """Process circle center and radius"""
    ...
    def _process_sphere_data(self, data: Dict[str, str]) -> List[str]:
        """Process sphere center and radius"""
    ...
    def _get_shape_code(self, shape: str, group: str) -> str:
        ...
        # Special handling for Point (dimension-dependent)
    ...
    def _values_str(self, group: str) -> str:
        """Format encoded values as string for keylog"""
        vals = self.results_A if group == 'A' else self.results_B
        return ('='.join(vals) + '=') if vals else ''

    def _generate_keylog(self) -> str:
        """
        Generate final keylog string for Casio calculator
        
        Returns:
            Keylog string in format compatible with target calculator version
        """
